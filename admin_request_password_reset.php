...
if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'admin') {
    header("Location: login_form.html"); // Redirect to login if not logged in or not admin
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($admin) {
        ...
        $stmt->execute([$admin['user_id'], $token, $expires_at]);
        ...
        try {
            ...
            $stmt->execute([$admin['user_id'], 'Requested admin password reset']);
...
<body>
    <h1>Admin Request Password Reset</h1>
