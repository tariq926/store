    ...
    if ($productId) {
        try {
            ...
            if ($existingCartItem) {
                ...
                $updateStmt = $pdo->prepare("UPDATE user_cart SET quantity = ? WHERE user_id = ? AND product_id = ?");
                $updateStmt->execute([$newQuantity, $userId, $productId]);
            } else {
                // Insert new cart item
                $insertStmt = $pdo->prepare("INSERT INTO user_cart (user_id, product_id, quantity) VALUES (?, ?, ?)");
                $insertStmt->execute([$userId, $productId, 1]); // Default quantity is 1
            }
...
function lazy_load_images() {
    ...
    try {
        ...
        echo "Error lazy loading images: " . $e->getMessage();
    }
}

// Best practices improvements
// Use prepared statements for database queries
function use_prepared_statements() {
    try {
        ...
        echo "Error using prepared statements: " . $e->getMessage();
    }
}
?>
<!DOCTYPE html>
